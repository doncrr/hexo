---
title: butterfly主题添加高斯模糊+说说界面魔改
tags:
  - 教程
  - 主题魔改
  - 美化
categories:
  - 折腾日记
mathjax: true
description: 感觉高斯模糊比普通透明模糊好看一点。。。说说魔改主要是转载，自己美化了一点
abbrlink: 8ab48c6c
date: 2024-08-12 17:12:14
cover: https://img1.dancihu.com/2024-02-19/db92acb0-5725-6746-0f1e-61f7cb9c6806.jpg
---
# 添加高斯模糊
新建css文件或直接添加到已有的css文件，注意在butterfly的配置文件中引用，代码如下：
```markdown
/* 首页文章卡片 */
...
# 说说界面魔改
## 前言
   主要是转载[leonus](https://blog.leonus.cn/)大佬的[这篇文章](https://blog.leonus.cn/2023/memeos.html)
   本人在此基础上又更改和美化了一些css代码，搭配高斯模糊体验更加，具体实现效果可以看我的说说界面。
## 正文
{% note warning modern %}注意：请在成功部署memos之后再添加到博客！！！！{% endnote %}
### 懒加载问题
...
```
### 实现
   首先通过`hexo n page xxx`创建页面，相信都很熟悉了。
   然后粘贴如下代码并按照注释修改内容。
```markdown
---
title: 说说
...
if(1) {
    ...
    fetch(url + '/api/v1/memo?creatorId=你的id&limit=显示文章数').then(res => res.json()).then(data => { // 注意修改域名和用户id
        let items = [],
            html = '',
